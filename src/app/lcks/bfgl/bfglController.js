...
angular.module('app')
.controller('lcksbfglController', ['$scope', '$rootScope', '$state', '$timeout', '$stateParams',
...
//			$scope.stateN = $scope.stateM.match(/[\u4e00-\u9fa5]/ig).join('');
			$scope.states = ['暂存(5)','待审核(15)','审核未通过(25)','审核通过待记账(35)','记账未通过(45)','已报废(55)'];
            $scope.btns = [
                ...
                {name:'scrap',con:'记账报废',class:'pos-fix-btn pf-btn-save btn-hover',show:false},
        ...
        $scope.divblur = function () {
            ...
            $scope.addScrap = function (){
                $state.go('main.lcks.bfgl.new',{state: 0});
                ...
                //     '</ul>'
                //     ,title: ['新建报废单','font-size: 14px;color: #fff;background-color: #555a69;line-height: 40px;padding: 0px 10px;border: none;textAlign: left;']
                //     ,closeBtn: 1
                //     ,shade: 0.3
                //     ,shadeClose: true
                //     ,btn: ['确认', '取消']
...
//			angular.element(".js-example-basic-single").select2();
            ...
            $scope.hrefs = function ($event) {
				...
				// window.location.href= 'http://dev.aek56.com/#/lcks/bfgl/new?state='+state;
                $state.go('main.lcks.bfgl.new',{state: state});
            }
            ...
            $scope.layerWin = function (type,conetnt){
                ...
                switch (type){
                    ...
                    case 'redo':
                        msg = '是否重新编辑？';
                        content = '操作成功！';
                        // return $state.go('main.lcks.bfgl.new',{state: 7});
                        break;
                }
                if(type == 'submit'){
                    msg = '提交后不可再修改，您确定提交吗？';
     ...
     //            	return;
                ...
                var index=layer.open({
                    ...
                    ,yes: function(index){
                        ...
                        if( type == 'redo'){
                            $state.go('main.lcks.bfgl.new',{state: 7});
                        }else{
                            $state.go('main.lcks.bfgl.bfgl');
                        }
                    }
                    ,area: ['500px','220px']
                    ,btnAlign: 'c'
