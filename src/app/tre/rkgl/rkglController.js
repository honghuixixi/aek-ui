...
angular.module('app')
.controller('rkglController', ['$scope', '$rootScope', '$state', '$timeout', '$stateParams',
...
//			$scope.stateN = $scope.stateM.match(/[\u4e00-\u9fa5]/ig).join('');
			$scope.states = ['暂存(5)','待审核(15)','审核未通过(25)','审核通过待记账(35)','记账未通过(45)','已入库(55)'];
            $scope.btns = [
                {name:'print',con:'打印入库单',class:'pos-fix-btn pf-btn-save',show:false},
                {name:'cancel',con:'取消',class:'pos-fix-btn pf-btn-save',show:false},
                {name:'save',con:'保存',class:'pos-fix-btn pf-btn-save',show:false},
                {name:'submit',con:'提交入库申请',class:'pos-fix-btn pf-btn-save btn-hover',show:false},
                ...
                {name:'account',con:'记账入库',class:'pos-fix-btn pf-btn-save btn-hover',show:false},
              {name:'noPass',con:'记账不通过',class:'pos-fix-btn pf-btn-save btn-hover',show:false},
               {name:'cancel',con:'取消入库',class:'pos-fix-btn pf-btn-save',show:false}
            ];
            $scope.trData = [
	  ...
	  //   	angular.element(".basic-multiple").select2({
            ...
            $scope.hrefs = function ($event) {
				...
				// window.location.href= 'http://dev.aek56.com/#/tre/rkgl/new?state='+state;
                $state.go('main.tre.rkgl.new',{state: state});
            }
...
//          	console.log(r);
   ...
   for(var i=0;i < arr.length;i++){
    ...
    if(!this.state){
       // angular.element('.pro-status').remove();
    }
    switch ($scope.state){
    	case 0:
     ...
     //    '<a name="print" class="pos-fix-btn pf-btn-save">打印入库单</a>');
    	...
    	default:
            ...
            $scope.toAssets = function () {
                $state.go('main.tre.rkgl.new',{state: 0});
            ...
            //     '</ul>'
            //     ,title: ['新建入库单','font-size: 14px;color: #fff;background-color: #555a69;line-height: 40px;padding: 0px 10px;border: none;textAlign: left;']
            //     ,closeBtn: 1
            //     ,shade: 0.3
            //     ,shadeClose: true
            //     ,btn: ['确认', '取消']
            ...
            $scope.layerWin = function (type,conetnt){
                ...
                switch (type){
                    ...
                    case 'cancel':
                        ...
                        break;
                    case 'redo':
                        return $state.go('main.tre.rkgl.new',{state: 7});
                        break;
                }
                if(type == 'submit'){
                    msg = '提交后不可再修改，您确定提交吗？';
