'use strict';
angular.module('app').controller('orgAddController', ['$scope' ,'$stateParams', '$rootScope', '$state', '$timeout', '$localStorage', '$http','$filter', function($scope, $stateParams, $rootScope, $state, $timeout, $localStorage,$http,$filter) {
    //底部高度设置，使其占满剩余全部
        ...
        $.ajax({
            ...
            complete:function (res) {
                if(res.responseJSON.code == 200){
                    ...
                    //医疗机构代码
                    $scope.hospCode = data.hplTenant.orgCode;


                    if(data.licenseImgUrl && data.licenseImgUrl!='string' && data.licenseImgUrl.trim()){
                        $scope.imgClick = true;
                        $scope.imgSrc = 'http://'+window.location.host + '/api/file'+data.licenseImgUrl;
                        $scope.fileUploadUrl = data.licenseImgUrl;
                    }
                    ...
                    if(res.responseJSON.data.tenant.customData){
                        ...
                        $scope.grade = {id:customData.grade,name:$scope.getAreaName(customData.grade,$scope.gradeArr)};
                        $scope.orgClassType = {id:customData.manageType,name:$scope.getAreaName(customData.manageType,$scope.orgClassTypeArr)};
