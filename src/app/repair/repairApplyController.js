angular.module('app')
    ...
    .controller('repairApplyController', ['$scope', '$stateParams', '$rootScope', '$state', '$timeout', '$localStorage', function($scope, $stateParams, $rootScope, $state, $timeout, $localStorage) {
        ...
        $scope.subApply = function(){
            ...
            if(!$scope.addList.deptId){
                var msg = layer.msg('<div class="toaster"><span>' + '该设备暂时无法提交维修申请，请稍后重试' + '</span></div>', {
                    area: ['100%', '60px'],
                    time: 3000,
                    offset: 'b',
                    shadeClose: true,
            ...
            if($scope.addList&&$scope.describe&&$scope.number&&$scope.addList.deptId){
                $scope.applyBtn = true;
                $.ajax({
                    type: 'post',
