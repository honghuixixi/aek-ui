...
.controller('detailController', ['$scope', '$stateParams', '$rootScope', '$state', '$timeout', '$localStorage','$filter', function($scope, $stateParams, $rootScope, $state, $timeout, $localStorage,$filter) {
	...
	$scope.showBig = function (tenant) {
		$scope.bigUrl = tenant.licenseImgUrl;
		$scope.showBigImg = true;
	}
	$scope.closeBig = function () {
		$scope.showBigImg = false;
	}
		...
		$.ajax({
			...
			complete: function(res) {
				if(res.responseJSON.code == 200) {
					...
					if(res.responseJSON.data.tenant.customData && res.responseJSON.data.tenant.customData != 'null') {
						...
						$scope.manageType = $scope.getTypeName(customData.manageType, $localStorage.baseOrg.tenantManageType);
					}
                    ...
                    var licenseImgUrl = res.responseJSON.data.tenant.licenseImgUrl;
                    if( licenseImgUrl && licenseImgUrl != 'string' && licenseImgUrl.trim()){
                        res.responseJSON.data.tenant.licenseImgUrl = 'http://'+window.location.host + '/api/file'+licenseImgUrl;
					}else{
                        res.responseJSON.data.tenant.licenseImgUrl = 'string';
                    }
	...
	$scope.orgSet = function() {
		...
		/*if($scope.nowOrgEnable) {
			...
			layer.msg('<div class="toaster"><span>当前医疗机构已被停用</span></div>', {
				area: ['100%', '60px'],
				time: 3000,
				offset: 'b',
				shadeClose: true,
	...
	$scope.orgOperate = "停用";
	$scope.tips = ["删除医疗机构后,下级医疗机构将一并删除且无法恢复，请谨慎操作！", "停用医疗机构将导致医疗机构对应后台用户均不可登录，该医疗机构及下级医疗机构后台页面不可访问，请谨慎操作。"];
	$scope.orgTip = "停用医疗机构将导致医疗机构对应后台用户均不可登陆，该医疗机构及下级医疗机构后台页面不可访问，请谨慎操作。";
