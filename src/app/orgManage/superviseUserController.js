...
.controller('superviseUserController', ['$scope', '$stateParams', '$rootScope', '$state', '$timeout', '$localStorage', function($scope, $stateParams, $rootScope, $state, $timeout, $localStorage) {
    ...
    $scope.getList = function() {
        ...
        $.ajax({
            ...
            // data: {tenantId:$stateParams.id,enable:$scope.modelId},
            data: {
		...
		if(n==0){
            ...
            $scope.changePwd = function () {
                ...
                $.ajax({
                    ...
                    data:JSON.stringify({password:$scope.userPwd}),
                    contentType: "application/json;charset=UTF-8",
                    complete: function(res) {
                        if(res.responseJSON.code == 200) {
	...
	$scope.showMoreOpt = function(i){
        ...
        $scope.resetPosition('.sele'+i,i);
	}

	//根据页面高度不同 用户操作菜单位置不同
    $scope.resetPosition = function (el,i) {
        var _winHeight = window.innerHeight,
            _thisTop = angular.element(el).offset().top;

        if(_winHeight-_thisTop-110 < 0){
            angular.element('.operateList'+i).css({
                'top':'inherit',
                'bottom':'8px'
            })
        }else{
