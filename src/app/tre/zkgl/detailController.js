...
angular.module('app')
    .controller('ZKGldetailController', ['$scope','$stateParams', '$rootScope', '$state', '$timeout', '$localStorage',
        function($scope,$stateParams,$rootScope, $state, $timeout, $localStorage) {
                    ...
                    complete: function(res) {
                        if(res.responseJSON.code == 200) {
                            ...
                            if($scope.billObj.billInfo.status!='待审核'){
                            	return $state.go('main.tre.zkgl.result', {billId: $stateParams.billId}, {
										reload: true
									});
                            }
                            $scope.billObj.applyInfo.assetCancleTxt='确定要撤销此设备转科吗？';
            ...
            $scope.power=$localStorage.userInfo.authoritiesStr.indexOf('ASS_ASSETS_ZK_CHECK') != -1;
            $scope.newTre=function(){
            	...
            	var index=layer.open({
		            ...
		            ,yes:function(){
		                ...
		                $.ajax({
		                    type: 'post',
		                    url: '/assets/assAssetsTransfer/auditTransfer',
		                    data: JSON.stringify($scope.adjust),
		                    contentType: "application/json;charset=UTF-8",
		                    complete: function(res) {
		                        if (res.responseJSON.code == 200) {
		                            $state.go('main.tre.zkgl.result', {billId: $stateParams.billId}, {
										reload: true
									});
		                        }else{
		                        	var msg = layer.msg('<div class="toaster"><i></i><span>'+res.responseJSON.msg+'</span></div>', {
