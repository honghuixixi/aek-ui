...
angular.module('app')
	.controller('assetsController', ['$scope', '$stateParams', '$rootScope', '$state', '$timeout', '$localStorage', 'FileUploader','$filter', 'printCodeService',
		function($scope, $stateParams, $rootScope, $state, $timeout, $localStorage, FileUploader,$filter, printCodeService) {
			var title = "";
...
// console.log($localStorage.userInfo)
			...
			$scope.btnData = [{
				limit:$rootScope.userInfo.authoritiesStr.indexOf('ASS_PREASSETS_EDIT') != -1,
				name: 'submit',
				text: '提交',
				class: 'btn-submit'
			}, {
			...
			$scope.saveAssets = function(callback, params, fn) {
				...
				if($scope.assetsDetailInfo.invoiceNos && $scope.assetsDetailInfo.invoiceNos.length){
					$scope.assetsDetailInfo.invoiceNos = $scope.assetsDetailInfo.invoiceNos.join(";");
				}
				$.ajax({
					type: "post",
	                        ...
	                        $.ajax({
	                            ...
	                            complete: function(xhr){
	                                if(xhr.responseJSON.code==200){
	                                	$state.go('main.tre.zctz.list',{id:$stateParams.id,isOp:true,isOpMsg:'操作成功！'});
	                                }else{
				...
				for(var i = 0; i < arr.length; i++) {
					...
					switch($stateParams.state) {
						...
						case '1':
							...
							$scope.pro4=false;
							$scope.btnData = [{
									limit:$rootScope.userInfo.authoritiesStr.indexOf('ASS_PREASSETS_VERIFY') != -1,
									name: 'pass',
									text: '验收',
									class: 'btn-submit'
								}
						...
						case '2':
							...
							$scope.btnData = [{
								...
								name: 'toAssets',
								text: '转为资产台账',
								class: 'btn-save btn-print toAssets'
							},{
								limit:true,
								name: 'print',
								text: '打印标签',
						...
						case '3':
                            ...
                            if($scope.currentState == 3) {
                            	// console.log($rootScope.userInfo.authoritiesStr)
                                $scope.btnData = [{
									limit:$rootScope.userInfo.authoritiesStr.indexOf('ASS_PREASSETS_EDIT') != -1,
                                    name: 'submit',
                                    text: '提交验收',
