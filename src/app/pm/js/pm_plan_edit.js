angular.module('app')
    .controller('pmPlanEditController', ['$rootScope', '$scope', '$stateParams', '$localStorage', '$state', 'pmDbService', 'pmUtilService', function($rootScope, $scope, $stateParams, $localStorage, $state, pmDbService, pmUtilService) {
        $scope.$parent.checkLimit('plan', function() {
            ...
            $scope.removeCheckUser = function () {
                ...
                $scope.data.plan.checkUser = { id: 0, realName: '', mobile: '' };
            };
            ...
            $scope.showCheckUserDialog = function() {
                $scope.data.checkUser = $scope.data.plan.checkUser;
                pmDbService.getCheckUsersForPlan(function(data) {
                    ...
                    layer.open({
                        ...
                        btn: ['确定', '取消'],
                        yes: function(index, layero) {
                            $scope.data.plan.checkUser = $scope.data.checkUser;
                            layer.close(index);
                            $scope.$apply();
                        }
                    });
            ...
            $scope.getPlanInfoForEdit = function() {
                pmDbService.getPlanInfoForEdit($stateParams.id, function(data) {
                    ...
                    if(data.check){
                        data.checkUser = { id: data.check.id, realName: data.check.name };
                    }
                ...
                if (flag) {
                    layer.open({
                        ...
                        yes: function(index, layero) {
                            ...
                            if($scope.data.plan.checkUser.id > 0){
                                param.check = {id: $scope.data.plan.checkUser.id, name: $scope.data.plan.checkUser.realName};
                            }
                            pmDbService.editPlan(param, function() {
                                $state.go("pm.menu.planinfo", { id: $stateParams.id });
                            });
                            layer.close(index);
                        }
                    });
