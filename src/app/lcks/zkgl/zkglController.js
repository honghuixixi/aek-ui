...
angular.module('app')
.controller('lckszkglController', ['$scope', '$rootScope', '$state', '$timeout', '$stateParams',
...
//			$scope.stateN = $scope.stateM.match(/[\u4e00-\u9fa5]/ig).join('');
			$scope.states = ['暂存(5)','待审核(15)','审核未通过(25)','审核通过待记账(35)','记账未通过(45)','已转科(55)'];
            $scope.btns = [
                {name:'print',con:'打印转科单',class:'pos-fix-btn pf-btn-save',show:false},
                {name:'cancel',con:'取消',class:'pos-fix-btn pf-btn-save',show:false},
                {name:'save',con:'保存',class:'pos-fix-btn pf-btn-save',show:false},
                {name:'submit',con:'提交转科申请',class:'pos-fix-btn pf-btn-save btn-hover',show:false},
                ...
                {name:'noPass',con:'审核不通过',class:'pos-fix-btn pf-btn-save',show:false},
                ...
                {name:'submit',con:'记账转科',class:'pos-fix-btn pf-btn-save btn-hover',show:false},
              {name:'noPass',con:'记账不通过',class:'pos-fix-btn pf-btn-save btn-hover',show:false},
               {name:'cancel',con:'取消转科',class:'pos-fix-btn pf-btn-save',show:false}
            ];
            $scope.trData = [
        ...
        $scope.divblur = function () {
            ...
            $scope.addTurn = function (){
                $state.go('main.lcks.zkgl.new',{state: 0});
                ...
                //     '</ul>'
                //     ,title: ['新建转科单','font-size: 14px;color: #fff;background-color: #555a69;line-height: 40px;padding: 0px 10px;border: none;textAlign: left;']
                //     ,closeBtn: 1
                //     ,shade: 0.3
                //     ,shadeClose: true
                //     ,btn: ['确认', '取消']
...
//			angular.element(".js-example-basic-single").select2();
            ...
            //             break;
            //         case 'redo':
            //             return $state.go('main.lcks.zkgl.new',{state: 7});
            //             break;
            //     }
            ...
            $scope.hrefs = function ($event) {
				...
				// window.location.href= 'http://dev.aek56.com/#/lcks/zkgl/new?state='+state;
                $state.go('main.lcks.zkgl.new',{state: state});
            }
     ...
     //            	return;
                ...
                var index=layer.open({
                    ...
                    ,yes: function(index){
                        ...
                        if(type == 'redo'){
                            $state.go('main.lcks.zkgl.new',{state: 7});
                        }else if(type == 'again'){
                            $state.go('main.lcks.zkgl.new',{state: 1});
                        }else{
                            $state.go('main.lcks.zkgl.zkgl');
                        }
                    }
                    ,area: ['500px','220px']
                    ,btnAlign: 'c'
